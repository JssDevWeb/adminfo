...
<body>
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 content-with-sidebar">
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">                        <form method="GET" class="row g-3">
                            ...
                            <div class="col-md-2">
                                <div class="d-grid h-100 align-items-end">
                                    <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#modalExportarPdf" 
    ...
    <div class="modal fade" id="modalExportarPdf" tabindex="-1" aria-labelledby="modalExportarPdfLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <form method="POST" action="procesar_pdf.php" target="_blank">
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-12">
                                <div class="alert alert-info">
                                    <i class="bi bi-info-circle"></i>
                                    <strong>Curso:</strong> <?php 
                                        if ($generar_graficos) {
                                            foreach ($cursos_disponibles as $curso) {
                                                if ($curso['id'] == $curso_grafico_id) {
                                                    echo htmlspecialchars($curso['nombre'] . ' - ' . $curso['descripcion']);
                                                    break;
                                                }
                                            }
                                        }
                                    ?><br>
                                    <strong>Fecha:</strong> <?php echo $fecha_grafico ? date('d/m/Y', strtotime($fecha_grafico)) : ''; ?>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <h6 class="mb-3">
                                    ...
                                    Seleccione las secciones a incluir en el PDF:
                                </h6>
                            </div>
                        </div>
                          <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-check mb-3">
                                    ...
                                    <label class="form-check-label" for="seccion_comentarios_curso">
                                        <strong><i class="bi bi-chat-quote"></i> Comentarios del Curso</strong>
                                        <small class="d-block text-muted">Comentarios cualitativos sobre el curso</small>
                                    </label>
                                </div>
                                ...
                                <div class="form-check mb-3">
                                    ...
                                    <label class="form-check-label" for="seccion_comentarios_profesores">
                                        <strong><i class="bi bi-chat-quote"></i> Comentarios de Profesores</strong>
                                        <small class="d-block text-muted">Comentarios cualitativos sobre profesores</small>
                                    </label>
                                </div>
                                
                                <div class="form-check mb-3">
                                    <input class="form-check-input" type="checkbox" name="secciones[]" value="preguntas_criticas" id="seccion_criticas" checked>
                                    <label class="form-check-label" for="seccion_criticas">
                                        <strong><i class="bi bi-exclamation-triangle"></i> Preguntas Críticas</strong>
                                        <small class="d-block text-muted">Preguntas con menores calificaciones y áreas de mejora</small>
                                    </label>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="col-12">
                                <div class="mt-3 p-3 bg-light rounded">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" id="seleccionar_todas" checked>
                                        <label class="form-check-label fw-bold" for="seleccionar_todas">
                        ...
                        <input type="hidden" name="curso_id" value="<?php echo htmlspecialchars($curso_grafico_id); ?>">
                        <input type="hidden" name="fecha" value="<?php echo htmlspecialchars($fecha_grafico); ?>">
                    </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-success" id="btnGenerarPdf">
    ...
    <script>
        // Script para el modal de exportar PDF
        document.addEventListener('DOMContentLoaded', function() {
            const checkboxSeleccionarTodas = document.getElementById('seleccionar_todas');
            const checkboxesSecciones = document.querySelectorAll('input[name="secciones[]"]');
            const btnGenerarPdf = document.getElementById('btnGenerarPdf');
            
            // Función para seleccionar/deseleccionar todas las secciones
            ...
            // Actualizar el estado del checkbox "Seleccionar todas" cuando cambian las secciones individuales
            checkboxesSecciones.forEach(checkbox => {
                checkbox.addEventListener('change', function() {
                    const todasSeleccionadas = Array.from(checkboxesSecciones).every(cb => cb.checked);
                    const ningunaSeleccionada = Array.from(checkboxesSecciones).every(cb => !cb.checked);
                    
                    if (todasSeleccionadas) {
                        checkboxSeleccionarTodas.checked = true;
                        checkboxSeleccionarTodas.indeterminate = false;
                    } else if (ningunaSeleccionada) {
                        checkboxSeleccionarTodas.checked = false;
                        checkboxSeleccionarTodas.indeterminate = false;
                    } else {
                        checkboxSeleccionarTodas.indeterminate = true;
                    }
            ...
            function actualizarEstadoBoton() {
                const algunaSeleccionada = Array.from(checkboxesSecciones).some(cb => cb.checked);
                btnGenerarPdf.disabled = !algunaSeleccionada;
                
                if (!algunaSeleccionada) {
                    btnGenerarPdf.innerHTML = '<i class="bi bi-exclamation-triangle"></i> Seleccione al menos una sección';
                    btnGenerarPdf.classList.remove('btn-success');
                    btnGenerarPdf.classList.add('btn-warning');
                } else {
                    btnGenerarPdf.innerHTML = '<i class="bi bi-download"></i> Generar y Descargar PDF';
                    btnGenerarPdf.classList.remove('btn-warning');
                    btnGenerarPdf.classList.add('btn-success');
                }
            }
            
            // Estado inicial
            actualizarEstadoBoton();
              // Agregar loading state al generar PDF
            document.querySelector('form[action="procesar_pdf.php"]').addEventListener('submit', function(e) {
                e.preventDefault(); // Prevenir envío inmediato
                
                btnGenerarPdf.innerHTML = '<i class="bi bi-hourglass-split"></i> Capturando gráficos...';
                btnGenerarPdf.disabled = true;
                
                // Capturar gráficos como imágenes antes de enviar
                capturarGraficosYEnviar(this);
            });        });
        
        /**
         * Función para capturar gráficos Chart.js como imágenes y enviar el formulario
         */
        function capturarGraficosYEnviar(form) {
            const imagenes_graficos = [];
            const canvases = document.querySelectorAll('canvas[id^="grafico_"]');
            
            if (canvases.length === 0) {
                // No hay gráficos, enviar directamente
                enviarFormularioConImagenes(form, imagenes_graficos);
                return;
            }
            
            let graficosCapturados = 0;
            const totalGraficos = canvases.length;
            
            canvases.forEach(function(canvas) {
                try {
                    // Obtener el título del gráfico desde el elemento padre
                    const graficoContainer = canvas.closest('.chart-container');
                    let titulo = 'Gráfico';
                    if (graficoContainer) {
                        const tituloElement = graficoContainer.querySelector('.chart-title, h5, h6');
                        if (tituloElement) {
                    ...
                    // Si ya capturamos todos los gráficos, enviar el formulario
                    if (graficosCapturados === totalGraficos) {
                        enviarFormularioConImagenes(form, imagenes_graficos);
                    }
                    
                } catch (error) {
                    console.error('Error capturando gráfico:', canvas.id, error);
                    graficosCapturados++;
        ...
        /**
         * Función para enviar el formulario con las imágenes de gráficos
         */
        function enviarFormularioConImagenes(form, imagenes_graficos) {
            const btnGenerarPdf = document.getElementById('btnGenerarPdf');
            
            try {
                // Crear un campo oculto para las imágenes
                const inputImagenes = document.createElement('input');
                inputImagenes.type = 'hidden';
                inputImagenes.name = 'imagenes_graficos';
                inputImagenes.value = JSON.stringify(imagenes_graficos);
                form.appendChild(inputImagenes);
                ...
                alert('Error al capturar los gráficos. Inténtelo de nuevo.');
